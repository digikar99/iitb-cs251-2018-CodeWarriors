...
if [ ! -d "$HOME/SPC" ] ; then 
    ...
    echo $HOME/SPC exists already...
fi

to_path="$(realpath linux-client.sh)"

if [ ! -f "$HOME/iitb-spc" ] ; then
...
if [ ! -d "$HOME/.config/iitb-spc" ] ; then
   printf "Creating $HOME/.config/iitb-spc/...\n"
   mkdir -p $HOME/.config/iitb-spc
fi
   
echo "Currently, we haven't included set up additional necessary tools. You'd
likely need to install curl."
